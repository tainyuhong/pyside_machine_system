...
async def save_file(name,file):
    async with aiofiles.open('d:\\txt\\{}'.format(name),'w+') as f:
        await f.write(file)
...
async def get_xiaoshou():
    ...
    async with aiohttp.ClientSession() as session:
        for item_url in menu_list:# print(item_url)
            async with session.get(item_url, headers=headers) as itemdata:
                await asyncio.sleep(3)
                # print(await itemdata.text())
                item_data = etree.HTML(await itemdata.text())     # .encode('iso-8859-1').decode('utf-8')
                # 获取内容位置
                content_list = item_data.xpath('//*[@id="container"]/div/div/div[2]')
                for x in content_list:
                    title = x.xpath('./h1/text()')
                    # print(title)
                    data = x.xpath('./div[2]/p/text()')     # 每一章内容
                    # print('章节信息：',''.join(data))    # 将列表拼接成字符串
                    t = asyncio.create_task(save_file(title[0]+'.txt',''.join(data)))
                    tasks.append(t)
