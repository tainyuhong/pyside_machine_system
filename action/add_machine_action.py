...
class UiAdd(Ui_add_machine_form, QtWidgets.QDialog):
    ...
    def get_room(self):
        ...
        :return: 机房列表
        """
        room_data = MachineRoom.select(MachineRoom.room_name)  # 查询父类不为空的分类
    ...
    def save_infos(self):
        ...
        room = self.room.currentIndex() + 1  # 机房名id
    ...
    def get_cabinet(self):
        ...
        cabinet_data = Cabinet.select(Cabinet.cab_num).where(Case(None, [
            (self.room.currentText() == 'ZB-1', Cabinet.room == '1'),
            (self.room.currentText() == 'ZB-2', Cabinet.room == '2'),
            (self.room.currentText() == 'ZB-3', Cabinet.room == '3'),
            (self.room.currentText() == 'ZB-4', Cabinet.room == '4')]))
        # print(cabinet_data)
